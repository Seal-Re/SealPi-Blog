...
- ~~**标签 CRUD 后端 API**~~: `GET /api/v1/tags` 已实现 (T17 ✓)
- ~~**浏览量递增**~~: `getSingleBySlug` 调用 `incrementViewCount` 已实现 (T18 ✓)
- ~~**用户管理后台**~~: `/admin/users` 已实现 (T19 ✓)
- ~~**前端测试框架**~~: Vitest 单元测试已引入 (T20 ✓)
- **阶段 C（监控面板进一步强化）**: 当前 ops 面板提供基础统计，高级监控（Grafana/告警）留到 v1.3+
- **Playwright E2E smoke**: 留到 v1.3
...
- [x] **T17** 后端独立标签聚合 API ✓
  - `GET /api/v1/tags` 已实现，`TagMapper.selectPublishedTagsWithCount()` 动态计算各标签下已发布文章数
  - 前端 `fetchPublishedTags()` 优先调用此端点，失败后回退全量文章聚合
- [x] **T18** 浏览量递增端点 ✓
  - `ArticleServiceImpl.getSingleBySlug()` 在每次文章详情读取时调用 `incrementViewCount()`
  - 前台文章详情页每次缓存 miss（5 分钟 ISR）都会触发递增
- [x] **T19** Admin 信息架构拆分 + 基础监控面板 ✓
  - `/admin/ops` 运维控制台：文章总数/已发布/草稿/注册用户四项统计
  - `/admin/users` 用户管理列表：头像、昵称、评论权限、注册/登录时间
  - `/admin/articles` 文章管理：分页、状态筛选、标签显示、在线/离线/删除操作
- [x] **T20** 前端测试框架（Vitest 单元测试）✓
  - 安装 vitest ^4.1.4，新增 `vitest.config.ts`
  - `lib/__tests__/article-status.test.ts`：13 个单元测试覆盖状态常量与判断函数
  - `lib/__tests__/admin-api.test.ts`：6 个单元测试覆盖 `AdminApiError` 行为
  - CI `frontend` job 增加 `npm test` 步骤（lint → test → build）
  - Playwright E2E smoke 留到 v1.3
